!> This program will process the data from a modal urban plume simulation
...
= "out/modal_urban_plume"
  ...
  real(kind=dp), allocatable :: times(:), diams(:), num_dist(:), &
    num_conc(:)
  type(stats_1d_t) :: stats_num_dist, stats_tot_num_conc, &
    stats_num_conc, stats_rates_0, stats_rates_3, stats_tot_mass_conc
  real(kind=dp), allocatable :: rates_0(:), rates_3(:)

  call pmc_mpi_init()

  ! TODO: avoid hardcoding times and i_index
  allocate(times(7))

...
= "out/sect_init_sig_2_5"
  ...
  do i_index = 1,13
    ...
    call input_sectional(in_filename, index, time, del_t, uuid, bin_grid=bin_grid, &
        ...
        gas_state=gas_state, env_state=env_state)
    ...
    if (i_index == 1) then
      ...
      do i=1,bin_grid_size(bin_grid)
        ...
        v_d(i) = scenario_loss_rate_dry_dep(vol, aero_data%density(1), &
      ...
      call scenario_section_dry_dep_rates(bin_grid, aero_data, env_state, &
        rates)
